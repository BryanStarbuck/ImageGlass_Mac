...
final class MCPServerTests: XCTestCase {
    ...
    func testToolsListIncludesAllSpecTools() throws {
        ...
        // Spec §6: every v1 tool must be present. Other agents add tools too
        // (external_tools, charter_status, etc.) — those are not part of this
        // assertion, but the spec set must always show up.
        let specV1: Set<String> = [
            "list_scopes",
            "get_scope",
            "create_scope",
            "set_directories",
        ...
        XCTAssertTrue(specV1.isSubset(of: names),
                      "missing spec tools: \(specV1.subtracting(names))")
